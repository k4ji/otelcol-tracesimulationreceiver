FROM otel/opentelemetry-collector-builder:0.136.0 as builder
WORKDIR /app
COPY . .
RUN CGO_ENABLED=0 ocb --config=./manifest.yaml

FROM alpine/git:2.47.2 AS fetcher
RUN git clone --depth=1 --branch v0.5.2 https://github.com/k4ji/tracesimulationreceiver.git /tmp/tracesim
